...
export class Unit4LT2Service {
  ...
  async getSankeyData(payload: {
    ...
    if (payload.startTime && payload.endTime) {
      ...
      const nextDay = moment(payload.endDate)
        .add(1, "day")
        .format("YYYY-MM-DD");
    ...
    // ---------------- Prepare Sankey Data ----------------
    const tf2 = +(consumptionTotals["U13_GW01_Del_ActiveEnergy"] || 0).toFixed(
      2
    );
    const GasGen = +(
      consumptionTotals["U11_GW01_Del_ActiveEnergy"] || 0
    ).toFixed(2);
    const Solar = +(
      consumptionTotals["U24_GW01_Del_ActiveEnergy"] || 0
    ).toFixed(2);
    const Solar52 = +(
      consumptionTotals["U28_PLC_Del_ActiveEnergy"] || 0
    ).toFixed(2);

    const totalGeneration = tf2 + GasGen + Solar + Solar52 + PDB12CD12_sum;
...
Object.keys(meterMap).forEach((m) => {
  ...
  let meterConsumption = +(Number(consumptionTotals[key] || 0).toFixed(2));

  if (m === 'U5_GW01') {
    // Replace with PDB1CD1_sum
    meterConsumption = +(Number(PDB1CD1_sum).toFixed(2));
  } 
  else if (m === 'U9_GW01') {
    // Replace with PDB2CD2_sum
    meterConsumption = +(Number(PDB2CD2_sum).toFixed(2));
  } 
  else if (m === 'U15_GW01') {
    // Subtract PDB10_U4
    meterConsumption = Math.max(0, +(meterConsumption - PDB10_U4).toFixed(2));
  }
  ...
  totalConsumption += meterConsumption;
    ...
    // Add export legs
    totalConsumption += PDB08_U4 + CardPDB1_U4 + ToU5LT2_sum;
    ...
    const sankeyData = [
      ...
      // NEW generation legs
      {
        from: "From U5LT1(ComberM/C1-14 & CardM/C8-14)",
        to: "TotalLT2",
        value: PDB12CD12_sum,
      }, // verified
      ...
      {
        from: "TotalLT2",
        to: "PDB08->To U5LT1(ComberM/C1-14)",
        value: PDB08_U4,
      }, // verified
      {
        from: "TotalLT2",
        to: "CardPDB1->To U5LT1(CardM/C8-14)",
        value: CardPDB1_U4,
      }, // verified
      {
        from: "TotalLT2",
        to: "PDB10->To U5LT2(AutoCone 10-18)",
        value: ToU5LT2_sum,
      }, // verified
