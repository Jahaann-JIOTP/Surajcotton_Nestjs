...
export class HarmonicsService
{
  ...
  async getHarmonicsFromPayload ( payload: HarmonicsPayload ): Promise<HarmonicsResponse>
  {
    ...
    {
      // Add interval information for hourly data
      finalPeriod1Data = this.addHourlyIntervalInfo( processedPeriod1Data );
      finalPeriod2Data = this.addHourlyIntervalInfo( processedPeriod2Data );
  ...
  ): Promise<any[]>
  {
    ...
    const pipeline = [
      ...
      {
        // For 6 AM to 6 AM grouping, subtract 6 hours from the timestamp for grouping
        $addFields: {
          adjustedTimestamp: {
            $dateSubtract: {
      ...
      {
        $addFields: {
          // Add 6 hours back to get the actual time (6 AM based)
          actualTimestamp: {
            $dateAdd: {
              startDate: "$groupTimestamp",
              unit: "hour",
              amount: 6,
              timezone: this.TIMEZONE
            }
      ...
      },
      {
        $project: {
          ...
          originalTimestamp: "$minTimestamp",
  ...
  private getGroupingStage ( resolution: string ): any
  {
    ...
    // Group by the adjusted timestamp (after subtracting 6 hours)
  ...
  private processHarmonicsData ( data: any[], deviceIds: string[] ): any[]
  {
    return data.map( item =>
    {
      const processedItem: any = {
        ...
        originalTimestamp: item.originalTimestamp || item.timestamp
      };
      ...
      deviceIds.forEach( deviceId =>
      {
        ...
        // Add ONLY the calculated averages to the processed item
        processedItem[ `${ deviceId }_Harmonics_V_THD` ] = Math.round( vAvg * 100 ) / 100;
        processedItem[ `${ deviceId }_Harmonics_I_THD` ] = Math.round( iAvg * 100 ) / 100;
      } );
  ...
  private addHourlyIntervalInfo ( data: any[] ): any[]
  {
    return data.map( item =>
    {
      ...
      // Create a copy of the item with interval information
      const itemWithInterval = {
        ...item,
        intervalStart: timestamp.format( 'YYYY-MM-DD HH:mm:ss' ),
        intervalEnd: timestamp.clone().add( 1, 'hour' ).format( 'YYYY-MM-DD HH:mm:ss' )
      };

      // Remove originalTimestamp if it exists
      delete itemWithInterval.originalTimestamp;

      return itemWithInterval;
    } );
  }
  ...
  private groupByDayWithMinMax ( data: any[], deviceIds: string[] ): any[]
  {
    ...
    // Group data by day from 6 AM to 6 AM
    data.forEach( item =>
    {
      const timestamp = moment.tz( item.timestamp, this.TIMEZONE );

      // Adjust for 6 AM to 6 AM grouping
      const adjustedTimestamp = timestamp.hour() < 6
        ? timestamp.clone().subtract( 1, 'day' )
        : timestamp.clone();

      const dayKey = adjustedTimestamp.format( 'YYYY-MM-DD' );
    ...
    Object.keys( groupedByDay ).sort().forEach( dayKey =>
    {
      ...
      // For 6 AM to 6 AM grouping, the interval starts at 6 AM
      const intervalStart = moment.tz( `${ dayKey } 06:00:00`, 'YYYY-MM-DD HH:mm:ss', this.TIMEZONE );
      const intervalEnd = intervalStart.clone().add( 24, 'hours' );
  ...
  async getRawHarmonicsData ( payload: HarmonicsPayload ): Promise<any>
  {
    ...
    if ( payload.resolution === 'hour' || payload.resolution === '1hour' )
    {
      // For hourly raw data, we need to aggregate it first
      finalPeriod1Raw = this.groupRawDataByHour( processedPeriod1Raw, payload.DeviceId );
      finalPeriod2Raw = this.groupRawDataByHour( processedPeriod2Raw, payload.DeviceId );
    } else if ( payload.resolution === 'day' || payload.resolution === '1day' )
    {
  ...
  // Group raw data by hour for raw data endpoint
  private groupRawDataByHour ( data: any[], deviceIds: string[] ): any[]
  {
    const groupedByHour: { [ key: string ]: any[] } = {};

    // Group data by hour from 6 AM to 6 AM
    data.forEach( item =>
    {
      const timestamp = moment.tz( item.timestamp, this.TIMEZONE );
      ...
      const hourKey = `${ adjustedTimestamp.format( 'YYYY-MM-DD' ) } ${ timestamp.format( 'HH:00:00' ) }`;
    ...
    Object.keys( groupedByHour ).sort().forEach( hourKey =>
    {
      ...
      const hourTimestamp = moment.tz( `${ datePart } ${ timePart }`, 'YYYY-MM-DD HH:mm:ss', this.TIMEZONE );

      const hourlyItem: any = {
        timestamp: hourTimestamp.format( 'YYYY-MM-DDTHH:mm:ss+05:30' ),
        intervalStart: hourTimestamp.format( 'YYYY-MM-DD HH:mm:ss' ),
        intervalEnd: hourTimestamp.clone().add( 1, 'hour' ).format( 'YYYY-MM-DD HH:mm:ss' )
      };

      // For each device, calculate averages for this hour
      deviceIds.forEach( deviceId =>
      {
        const vValues = hourData
          .map( item => item[ `${ deviceId }_Harmonics_V_THD` ] )
          .filter( value => value !== undefined && value !== 0 );

        const iValues = hourData
          .map( item => item[ `${ deviceId }_Harmonics_I_THD` ] )
          .filter( value => value !== undefined && value !== 0 );
